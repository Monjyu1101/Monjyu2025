...
# MCPクライアントのインスタンス化
mcp_client = _v7__mcp_client.mcp_client_class()


async def main():
    """異なる起動方法でMCPサーバーの動作をテストします"""
    logger.info("MCPクライアントを起動します")
    ...
    if True:
        ...
        # モジュールとして起動
        start_success = await mcp_client.start_module(module_path, parms, env)
        if start_success:
        ...
        # HelloWorldツールのテスト
        result = await mcp_client.execute("_v7__mcp_server/HelloWorld")
        if result:
    ...
    if True:
        ...
        # スクリプトとして起動
        start_success = await mcp_client.start_script(module_path, parms, env)
        ...
        result = await mcp_client.execute("_v7__mcp_server/Echo", request_json=request_json)
        if result:
    ...
    if True:
        ...
        # 設定ファイルから起動
        start_success = await mcp_client.start_from_config(config_path)
        ...
        request = {"path": "."}
        request_json = json.dumps(request, ensure_ascii=False)
        result = await mcp_client.execute("filesystem/list_directory", request_json=request_json)
        if result:
